...
% load battparams_22Age_0mesh_MOL.mat
...
fy21 = @(t,y) [(vocv - y(1)*batt.R0 - y(2) - vbatt(t))/batt.L0;...
              ...
              y(2)/(batt.Rsei*batt.Cdl) - y(3)/(batt.Cdl)*(1/batt.Rsei + 1/batt.Rct) + y(4)/(batt.Rct*batt.Cdl);...
              y(3)/(batt.Rct*batt.Cw) - y(4)/(batt.Cw*batt.Rct)];
          
fy22 = @(t,y) [-y(1)/(batt.Rsei*batt.Csei) + y(2)/(batt.Rsei*batt.Csei) + ibatt(t)/batt.Csei;...
              y(1)/(batt.Rsei*batt.Cdl) - y(2)/(batt.Cdl)*(1/batt.Rsei + 1/batt.Rct) + y(3)/(batt.Rct*batt.Cdl);...
              y(2)/(batt.Rct*batt.Cw) - y(3)/(batt.Cw*batt.Rct)];

%% solutions
ts = 0; tf = 3600;
y021 = [0; 0; 0; 0]; y022 = [0; 0; 0]; % initital conditions
...
load SPICE_output_4_states_vbatt
t_SPICE = SPICEoutput4statesvbatt(:,1);
Ibatt_SPICE = SPICEoutput4statesvbatt(:,2); 
Vsei_SPICE = SPICEoutput4statesvbatt(:,3);
Vdl_SPICE = SPICEoutput4statesvbatt(:,4);
Vw_SPICE = SPICEoutput4statesvbatt(:,5);
Vbatt_SPICE = SPICEoutput4statesvbatt(:,6);

figure(5) % case 1 plots
tiledlayout(3,4)
...
nexttile;
plot(t21_ode15s,y21_ode15s(:,4),t_SPICE,Vw_SPICE,'--')
grid on
ylabel('$v_{w}(t)$'); xlabel('$t$')
ax5 = figtex(gca);
